(function () {
  ...
  // Bounds worst-case work per trigger so a pathological or malformed vein
  ...
  // (same numeric block id, so it only chains within a single ore type),
  ...
  api.on('blockChange', (payload) => {
    ...
    // Identify "ore" generically: requires a pickaxe of some minimum tier,
    // rather than hardcoding block keys (so modded ores are picked up too).
    const blockDef = V.Blocks[before];
    if (!blockDef) return;
    if (!blockDef.requiresTool || blockDef.tool !== 'pickaxe' || !(blockDef.minTier > 0)) {
      return;
    }
    ...
    // base game; only chain out into its still-standing same-id neighbors.
    const seeds = [
      [x + 1, y, z], [x - 1, y, z],
      [x, y + 1, z], [x, y - 1, z],
      [x, y, z + 1], [x, y, z - 1]
    ].filter(([nx, ny, nz]) => world.getBlock(nx, ny, nz) === matchId);

    if (seeds.length) {
      floodFillBreak(game, world, seeds, matchId);
    }
  });
})();
